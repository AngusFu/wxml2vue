...
it('should parse <template is="name" data="*"></template>', async () => {
  const array = [
    ...
    [
      ...
      '<template name="A" props="{ x }"><text :id="`w-${x}`"> A template </text></template>'
    ],
    // simple text interpolation
    [
      `<template name="A"><div>{{ www }}<span>w-{{ yyy }}</span></div></template>`,
      `<template name="A" props="{ www, yyy }"><div>{{ www }}<span>w-{{ yyy }}</span></div></template>`
    ],
    // complex text interpolation
    [
      `<template name="A"><div>{{ z[w + '___'] }}</div></template>`,
      `<template name="A" props="{ w, z }"><div>{{ z[w + '___'] }}</div></template>`
    ],
    // simple wx:if directive
    [
      `<template name="A"><div wx:if="{{eee}}"></div></template>`,
      `<template name="A" props="{ eee }"><div v-if="eee"></div></template>`
    ],
    // complex wx:if directive
    [
      `<template name="A"><div wx:if="{{ eee ? x : y }}"></div></template>`,
      `<template name="A" props="{ eee, x, y }"><div v-if="eee ? x : y"></div></template>`
    ],
    //  simple wx:for directives
    [
      `<template name="A"><div wx:for="{{ array }}">{{item}}</div></template>`,
      `<template name="A" props="{ array }"><div v-for="(item, index) in array">{{item}}</div></template>`
    ],
    //  complex wx:for directives
    [
      `<template name="A"><div wx:for="{{ w ? array1 : [...array2, 100] }}"></div></template>`,
      `<template name="A" props="{ array1, array2, w }"><div v-for="(item, index) in (w ? array1 : [...array2, 100])"></div></template>`
    ],
    ...
    [
      ...
      `<template name="A" props="{ array, xxx }">
        {{ xxx }}
        <div v-for="(it, $index) in array">{{it}}{{$index}}</div>
      </template>`
    ],
    ...
    [
      ...
      `<template name="A" props="{ array, ccc, eee, xxx }">
        <div v-for="(it, $index) in array">{{it}}{{$index}}</div>
        <template name="B" props="{ array, bbb, ccc }">
          <div :class="\`x-\${ccc}\`">{{ bbb }} {{ array.length }}</div>
        </template>
